...
# %% [markdown]
# # Make 64x64 image classification dataset
...
trainval_df_classif = pd.concat(_dfs)

for label, items in stratification:
    print(label, trainval_df_classif[label].value_counts())

# %%
# NPZs
it = tqdm.tqdm(trainval_df_classif.itertuples(), total=len(trainval_df_classif))

trainval_items = joblib.Parallel(n_jobs=10)(
    parallel_classification_sample(trainval_dataset, item) for item in it
)
...
eval_df_classif = pd.concat(_dfs)

for label, items in stratification:
    print(label, eval_df_classif[label].value_counts())

# %%
# NPZs
it = tqdm.tqdm(eval_df_classif.itertuples(), total=len(eval_df_classif))

eval_items = joblib.Parallel(n_jobs=10)(
    parallel_classification_sample(eval_dataset, item) for item in it
)
...
dataset_path = Path("./data") / "toy_cloud_classification_2025.npz"
...
f = ds.open(
    "https://storage.googleapis.com/fchouteau-isae-deep-learning/toy_cloud_classification_2025.npz",
    "rb",
)
...
ax.imshow(grid)
ax.axis("off")
plt.savefig("cloud_samples_classification.png")
plt.show()
